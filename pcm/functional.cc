...
namespace LaDa
{
  ...
  { 
    ...
    void Functional :: pack_gradients( t_GradientArg _grad) const
    {
      ...
      if( relaxation == Relaxation::default_ )
      {
        math::rMatrix3d const stress( -forces.cell * (~(!structure.cell)) );
        for( size_t i(0); i < 3; ++i )
