...
.. note:: It is also possible to tell the :py:class:`Vasp <functional.Vasp>`
          ...
          >>> vasp.program = '/path/to/vasp'

          It will apply only to calculations launched with that particular
          instance. At this point, however, it cannot be
...
<functional.vasp.encut>` is one of those. It can be specified:
  ...
  - as floating point signed by a quantity:
    ...
    In that case, the result will correctly converted to electron-Volts in the INCAR.
...
Below, a method based on the secant_ method is presented:
  ...
  I. Find interval for out-of-plane relaxed lattice parameter 

    1. Compute the input structure 
    2. Check if out-of-plane stress is positive or negative
    3. Increase/decrease out-of-plane lattice parameter
    4. Iterate until interval found

  II. Bisect interval

    1. Compute structure for out-of-plane lattice paramater in middle of interval
    2. Check if out-of-plane stress is positive or negative
    3. Adjust interval accordingly
