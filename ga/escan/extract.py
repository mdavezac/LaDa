...
class Extract(object): 
  ...
  def input(self): 
    ...
    assert exists(join(self.directory, self.OUTCAR)), \
           IOError("Could not find file {0}.".format(join(self.directory, self.OUTCAR)))
    return read_input\
           (\
             join(self.directory, self.OUTCAR),\
  ...
  def functional(self): 
    ...
    assert exists(join(self.directory, self.FUNCCAR)), \
           IOError("Could not find file {0}.".format(join(self.directory, self.FUNCCAR)))
    funccar = read_input(join(self.directory, self.FUNCCAR) )
  ...
  def ages(self): 
    ...
    for name in self.ordinals:
      filepath = join(join(self.directory, name), self.OUTCAR)
  ...
  def move_to_new_age(self): 
    ...
    if not exists(join(self.directory, "stdout")): return
    newdir = join(self.directory, self.next_dirname)
    if not exists(newdir): makedirs(newdir)
    for file in ["stdout", "stderr"]:
      move(join(self.directory, file), join(newdir, self.OUTCAR))
    for file in [self.INCAR, self.FUNCCAR]: 
      copy(join(self.directory, file), newdir)
