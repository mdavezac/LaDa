...
def nbstates( _escan, _structure, _addstates ):
  """
...
def vecnorm( _vec ):
  import LaDa
  from math import sqrt
  return sqrt( LaDa.norm2(_vec) )

def print_bands( _escan, _structure, _ocell, _first, _last,\
                 _result, _nbpoints = 10, _offset = 0, _sign = 1 ):
  """
...
def compute_allbands( _mpi, _vff, _escan, _structure, _kpoint, _scale, _perpdir, _add_states ):
    ...
    nbstates( _escan, _vff.structure, _add_states )
...
# save_strings_to_file( "ge_bands", results )
  ...
  print_bands( escan, vff.structure, structure.cell,
               X, W, result, 20 )
  print_bands( escan, vff.structure, structure.cell,
               W, G, result, 20, 2.0*vecnorm(W), -1 )

  file = open( "si_band_structure", "w" )
  for eigs in result:
    string = ""
    for u in eigs:
      string += " %f " % ( u )
