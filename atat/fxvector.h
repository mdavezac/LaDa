...
*/
    ...
    types::t_real normalize(void) {
      types::t_real norm=0.;
      { FORALLi { norm+=(types::t_real)(x[i]*x[i]); } }
      norm=sqrt(norm);
...
ostream& operator<<(ostream& s, const FixedVector<T,2> &v) {
  s << std::setw(10) << v.x[0] << " " << std::setw(10) << v.x[1];
  return s;
}
...
*/
    ...
    T& operator()(types::t_int i,types::t_int j) {return x[i][j];}
    const T& operator()(types::t_int i,types::t_int j) const {return x[i][j];}
    FixedMatrix<T,D> operator-(void) const {
      RESULT_FORALLij( result.x[i][j]=-x[i][j] );
    }
    void set_column(types::t_int column, const FixedVector<T,D> &vector) {
      FORALLi {x[i][column]=vector.x[i];}
    }
    void set_row(types::t_int row, const FixedVector<T,D> &vector) {
      FORALLi {x[row][i]=vector.x[i];}
    }
    FixedVector<T,D> get_column(types::t_int column) const {
      FixedVector<T,D> result;
      FORALLi {result.x[i]=x[i][column];}
      return result;
    }
    FixedVector<T,D> get_row(types::t_int row) const {
      FixedVector<T,D> result;
...
template<class T, types::t_int D>
  ...
  public:
    ...
    types::t_int is_in(const FixedVector<T,D> &v) const {
      FORALLi {
