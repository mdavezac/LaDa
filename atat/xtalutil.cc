...
rVector3d flip_into_brillouin_1(const rVector3d &k, const rMatrix3d &rec_lat) {
  ...
  rVector3d closest_k;
  types::t_real min_norm=MAXFLOAT;
  for (;c; c++) {
    rVector3d cur_k=k+rec_lat*to_real(c);
    types::t_real norm_k=norm(cur_k);
    if (norm_k<min_norm) {
      closest_k=cur_k;
      min_norm=norm_k;
    }
...
iVector3d make_miller(rVector3d normal) {
  ...
  iVector3d miller;
  for (types::t_int i=0; i<3; i++) {
    miller(i)=iround(normal(i)*(types::t_real)common);
  }
  return miller;
}
...
void calc_lattice_vectors(rMatrix3d *lattice_vector, const rVector3d &cell_length, rVector3d cell_angle) {
  ...
  types::t_real x=sqrt(sqr(cell_length(2))-norm2(solution))/norm(axb);
...
types::t_int equivalent_mod_cell(const MultiCluster &a, const MultiCluster &b, const rMatrix3d &inv_cell) {
  ...
  for (types::t_int i=0; i<a.clus.get_size(); i++) {
    if (equivalent_mod_cell(a.clus(0),b.clus(i),inv_cell)) {
	  ...
	  types::t_int at=which_atom(b.clus,a.clus(j)+shift);
...
void LatticePointIterator::find_new_neighbors(types::t_real new_max_radius) {
  iVector3d max_cell=find_sphere_bounding_box(cell,new_max_radius);
  MultiDimIterator<iVector3d> c(-max_cell,max_cell);
  for (;c;c++) {
    rVector3d x=cell*to_real(c);
    types::t_real r=norm(x);
...
void LatticePointIterator::init(const rMatrix3d &_cell, types::t_int skipzero) {
  list.delete_all();
  cell=_cell;
  inc_radius=0;
  for (types::t_int i=0; i<3; i++) {
...
void find_all_atom_in_supercell(Array<rVector3d> *psuper_atom_pos,
  ...
  types::t_int nb_atom_supercell=iround(atom_pos.get_size()*fabs(det(supercell)/det(cell)));
...
void find_all_atom_in_supercell(Array<rVector3d> *psuper_atom_pos,
  ...
  types::t_int nb_atom=atom_pos.get_size();
  types::t_int nb_atom_supercell=nb_atom*iround(fabs(det(supercell)/det(cell)));
  rMatrix3d axes_to_super=!supercell;

  Array<rVector3d> &super_atom_pos=*psuper_atom_pos;
  Array<types::t_int> &super_atom_type=*psuper_atom_type;
  ...
  if (psuper_atom_type) super_atom_type.resize(nb_atom_supercell);
  {
	...
	for (which_cell(2)=box.min(2); which_cell(2)<=box.max(2)+zero_tolerance; which_cell(2)+=1.) {
	  for (types::t_int at=0; at<nb_atom; at++) {
...
void AtomPairIterator::init(void) {
  max_dist_in_cell=0.;
  for (types::t_int i=0; i<atom_pos1->get_size(); i++) {
    for (types::t_int j=0; j<atom_pos2->get_size(); j++) {
...
void AtomMultipletIterator::init(const rMatrix3d &_cell, const Array<rVector3d> &_atom_pos, types::t_int _ntuple) {
  cell=_cell;
  atom_pos=&_atom_pos;
  ntuple=_ntuple;
  ...
  if (ntuple>1) {
    ...
    for (types::t_int i=0; i<ntuple-2; i++) {pairs(i).init(cell,pairs(ntuple-2)(0),*atom_pos);}
    (*this)++;
  }
}
...
void AtomMultipletIterator::operator++(int) {
  while (1) {
    ...
    if (i==pairs.get_size()-1) {
      pairs(i)++;
      for (types::t_int j=0; j<i; j++) {
	pairs(j).init(cell,pairs(ntuple-2)(0),*atom_pos);
      }
    }
    types::t_real rmin=pairs(0).length();
    types::t_real rmax=pairs(ntuple-2).length();
    for (i=0; i<pairs.get_size(); i++) {
      types::t_int j;
      for (j=i+1; j<pairs.get_size(); j++) {
        types::t_real r=norm(pairs(i)(1)-pairs(j)(1));
  ...
  for (types::t_int i=0; i<multiplet.get_size()-1; i++) {
    multiplet(i+1)=pairs(i)(1);
  }
}

types::t_real get_cluster_length(const Array<rVector3d> &c) {
  types::t_real max_length=0.;
  for (types::t_int i=0; i<c.get_size(); i++) {
    for (types::t_int j=i+1; j<c.get_size(); j++) {
      types::t_real l=norm(c(i)-c(j));
