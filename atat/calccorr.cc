...
template <> GenericPlugIn<CorrFuncTable> *GenericPlugIn<CorrFuncTable>::list=NULL;
...
void calc_corr_to_conc(Array2d<Real> *pmat, const Structure &lat, const Array<Array<int> > &site_type_list, const SpaceGroup &spacegroup, const Array<Array<Array<Real> > > &corrfunc, int usemultcorr) {
  ...
  */
  ...
  for (int i=0; i<str.atom_type.get_size(); i++) {
    str.atom_type(i)=site_type_list(str.atom_type(i)).get_size();
  }
  MultiClusterBank point(str,1,spacegroup);
  ...
  for ( ; endpt; endpt++) {
    ...
    for ( ; c; c++) {
      ...
      if (usemultcorr) {corr*=calc_multiplicity(*c,spacegroup.cell,spacegroup.point_op,spacegroup.trans);}
