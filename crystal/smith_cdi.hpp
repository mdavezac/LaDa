  ...
  namespace crystal
  {
                               ...
                               math::rMatrix3d const &_supercell )
    {
      ...
      { 
        ...
        _self->matrix = left.cast<types::t_real>() * (!_lattice);
        _self->vector = smith.diagonal();
        return true;
      }
    ...
    static int smithtransform_init(SmithTransformData* _self, PyObject* _args, PyObject *_kwargs)
    {
      ...
      static char *kwlist[] = { const_cast<char*>("lattice"), const_cast<char*>("supercell") };
      ...
      else if(not python::convert_to_matrix(lattice, cell)) return -1;
      if(PyStructure_Check("supercell")) bigcell = ((StructureData*)supercell)->cell;
      else if(not python::convert_to_matrix(supercell, bigcell)) return -1;
