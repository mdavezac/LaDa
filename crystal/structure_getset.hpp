...
static PyObject* LADA_NAME(getscale)(LADA_TYPE *_self, void *closure)
  { Py_INCREF(_self->scale); return _self->scale; }
...
static int LADA_NAME(setscale)(LADA_TYPE *_self, PyObject *_value, void *_closure)
{
  ...
  // checks for accepted types.
  bool const isfloat = PyFloat_Check(_value);
  bool const isint = PyInt_Check(_value);
  bool const isarray = PyArray_Check(_value);
  if( not (isfloat or isint or isarray) )
  {
    LADA_PYERROR(TypeError, "Input to scale is not an acceptable type.");
    return -1;
  }
  // Loads quantity module.
  PyObject *quant = PyImport_ImportModule("quantities.unitquantity");
  if(quant == NULL) return -1;
  PyObject* quant_type = PyObject_GetAttrString(quant, "UnitQuantity");
  if(quant_type == NULL) {Py_DECREF(quant); return -1; }

  // check if is an instance
  bool const isinstv = PyObject_IsInstance(_value, quant_type) == 1;
  bool const isinst = PyObject_IsInstance(_self->scale, quant_type) == 1;
  Py_DECREF(quant_type);
  Py_DECREF(quant);
  if(isinstv) 
  {
