    ...
    INCOLLAPSE2(template< class T_MATRIX, class T_VECTOR > void) 
     ...
     INCOLLAPSE2( template< class T_MATRIX, class T_VECTOR > void )
       :: operator()( T_MATRIX &_A, T_VECTOR &_b, types::t_unsigned _dim )
       {
         ...
         const bblas::matrix_column< t_OMatrix > column0( coefficients(), 0 );
     ...
     INCOLLAPSE( opt::ErrorTuple ) :: evaluate()
     {
       ...
       typedef const bblas::vector_range< bblas::matrix_column<t_OMatrix> > t_Ecis;
       t_Ecis ecis( bblas::matrix_column< t_OMatrix>( coefficients(), 0 ), 
     ...
     INCOLLAPSE( void ) ::  init()
     {
       ...
       const bblas::range columns( 0, t_ColBase::dimensions() );
