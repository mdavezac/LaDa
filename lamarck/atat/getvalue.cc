...
int get_values(istream &s, int nb, AskStruct *label) {
  ...
  while (1) {
    ...
    int i;
    for (i=0; i<nb; i++) {
      if (strcmp(label[i].shortname,cur_label.str().c_str())==0) {
	...
	value_str << '\0';
	istringstream value(value_str.str().c_str());
	switch (label[i].vartype) {
	  ...
	  break;
        case STRINGVAL:
	  {
	    AutoString *pstr=new AutoString(value_str.str().c_str());
