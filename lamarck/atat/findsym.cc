...
void apply_symmetry(MultiCluster *b, const rMatrix3d &point_op, const rVector3d &trans, const MultiCluster &a) {
  ...
  b->func=a.func;
  for (types::t_int i=0; i<a.clus.get_size(); i++) {
    (b->clus)(i)=point_op*a.clus(i)+trans;
  }
}

types::t_int equivalent_by_symmetry(const rVector3d &a, const rVector3d &b,
...
types::t_int equivalent_by_symmetry(const Array<rVector3d> &a, const Array<rVector3d> &b,
                           const rMatrix3d &cell,
                           const Array<rMatrix3d> &point_op, const Array<rVector3d> &trans) {
  rMatrix3d inv_cell=!cell;
  for (types::t_int op=0; op<point_op.get_size(); op++) {
...
types::t_int equivalent_by_symmetry(const MultiCluster &a, const MultiCluster &b,
                           const rMatrix3d &cell,
                           const Array<rMatrix3d> &point_op, const Array<rVector3d> &trans) {
  rMatrix3d inv_cell=!cell;
  for (types::t_int op=0; op<point_op.get_size(); op++) {
...
void calc_concentration(types::t_int *pmin_type, types::t_int *pmax_type, Array<types::t_real> *pconc, const Array<types::t_int> &atom_type) {
  types::t_real dc=1./(types::t_real)atom_type.get_size();
  *pmin_type=min(atom_type);
  *pmax_type=max(atom_type);
  pconc->resize((*pmax_type)-(*pmin_type)+1);
  zero_array(pconc);
  for (types::t_int i=0; i<atom_type.get_size(); i++) {
    (*pconc)(atom_type(i)-(*pmin_type))+=dc;
  }
}

types::t_real check_sum_structure(const Array<types::t_int> &atom_type) {
  types::t_int sum=0;
  for (types::t_int i=0; i<atom_type.get_size(); i++) {sum+=atom_type(i);}
  return (types::t_real)sum/(types::t_real)atom_type.get_size();
}

types::t_int equivalent_by_symmetry(const Structure &a, const Structure &b,
                           const rMatrix3d &cell,
                           const Array<rMatrix3d> &point_op, const Array<rVector3d> &trans) {
  ...
  Array<types::t_real> conca,concb;
  types::t_int min_typea,max_typea,min_typeb,max_typeb;
  ...
  for (types::t_int i=0; i<conca.get_size(); i++) {
    ...
    if (is_int(inv_acell*tb.cell)) {
      ...
      for (;shift; shift++) {
        ...
        for (at=0; at<b.atom_pos.get_size(); at++) {
          types::t_int at2=which_atom(a.atom_pos,tb.atom_pos(at)+shift,inv_acell);
          if (at2==-1) break;
...
void find_primitive_unit_cell(rMatrix3d *pcell, const Structure &lat) {
  ...
  for (types::t_int i=0; i<3; i++) {pcell->set_column(i,a[i]);}
}

void find_spacegroup(Array<rMatrix3d> *point_op, Array<rVector3d> *trans,
                      const rMatrix3d &cell,
                      const Array<rVector3d> &atom_pos, const Array<types::t_int>
&atom_type) {
  rMatrix3d inv_cell=!cell;
  types::t_int max_type,min_type;
  Array<types::t_real> atom_type_conc;
  calc_concentration(&min_type,&max_type,&atom_type_conc,atom_type);
  types::t_int rare_type=-1;
  types::t_int min_conc=MAXINT;
  for (types::t_int i=0; i<atom_type_conc.get_size(); i++) {
  ...
  for (types::t_int i=ref_atom; i<atom_pos.get_size(); i++) {
    if (atom_type(i)==rare_type) {
      for (types::t_int op=0; op<pointgroup.get_size(); op++) {
        rVector3d trans=-pointgroup(op)*atom_pos(ref_atom)+atom_pos(i);
	  types::t_int at;
        for (at=0; at<atom_pos.get_size(); at++) {
          types::t_int at2=which_atom(atom_pos,pointgroup(op)*atom_pos(at)+trans,inv_cell);
          if (at2<0) break;
...
void find_all_equivalent_cell(Array<rMatrix3d> *psupercell, const rMatrix3d &cell, types::t_real radius) {
  ...
  types::t_real d=fabs(det(cell));
  LatticePointIterator lp[3];
  rMatrix3d curcell;
  types::t_real radius2=radius*radius+zero_tolerance;
  lp[2].init(cell,1);
...
void find_supercells(Array<rMatrix3d> *supercell, types::t_int min_volume, types::t_int max_volume, rMatrix3d unitcell, const Array<rMatrix3d> &pointgroup) {
  ...
  for (types::t_int volume=min_volume; volume<=max_volume; volume++) {
    types::t_real maxa=pow((types::t_real)volume*sqrt(2.),1./3.);
    types::t_int imaxa=(types::t_int)ceil(maxa);
    ...
    for ( ; cur_a; cur_a++) {
      iVector3d a=*cur_a;
      types::t_real len_a=norm(a);
      types::t_real maxb=sqrt(sqr(len_a)/6.+sqrt(sqr(sqr(len_a)/6.) + 4./3.*sqr((types::t_real)volume/len_a)));
      types::t_int imaxb=(types::t_int)ceil(maxb);
	  ...
	  types::t_real proj=(types::t_real)((iVector3d &)b*(iVector3d &)a)/(types::t_real)((iVector3d &)a*(iVector3d &)a);
	  iVector3d axb=(iVector3d &)a^(iVector3d &)b;
	  types::t_int axb2=norm2(axb);
	  if (proj>=(-0.5+   zero_tolerance) &&
	      proj<=( 0.5+2.*zero_tolerance) &&
	      axb2!=0) {
	    types::t_int minw=-axb2/2;
	    types::t_int maxw=-axb2/2+axb2;
	    for (types::t_int wa=minw; wa<maxw; wa++) {
	      for (types::t_int wb=minw; wb<maxw; wb++) {
  ...
  LinkedListIterator<iMatrix3d> cur_abc(list_abc);
  for (types::t_int i=0; i<supercell->get_size(); i++,cur_abc++) {
...
void find_supercells_2D(Array<rMatrix3d> *supercell, types::t_int min_volume, types::t_int max_volume, rMatrix3d unitcell, const Array<rMatrix3d> &pointgroup) {
  ...
  for (types::t_int volume=min_volume; volume<=max_volume; volume++) {
    types::t_real maxa=pow((types::t_real)volume*2./sqrt(3.),1./2.);
    types::t_int imaxa=(types::t_int)ceil(maxa);
    LinkedList<iVector3d> list_a;
    ...
    for ( ; cur_a; cur_a++) {
      iVector3d a=*cur_a;
      iVector3d cxa=c^a;
      types::t_int cxa2=norm2(cxa);
      types::t_int minw=-cxa2/2;
      types::t_int maxw=-cxa2/2+cxa2;
      for (types::t_int wa=minw; wa<maxw; wa++) {
...
void sort_supercells(Array<rMatrix3d> *p_supercell, const Array<rMatrix3d> &pointgroup) {
  Array<types::t_int> nb_sym_op(p_supercell->get_size());
  zero_array(&nb_sym_op);
  for (types::t_int i=0; i<nb_sym_op.get_size(); i++) {
    rMatrix3d &cell=(*p_supercell)(i);
    rMatrix3d icell=!cell;
    for (types::t_int j=0; j<pointgroup.get_size(); j++) {
      if (is_int(icell*pointgroup(j)*cell)) nb_sym_op(i)++;
    }
  }
  for (types::t_int i=1; i<nb_sym_op.get_size(); i++) {
    for (types::t_int j=nb_sym_op.get_size()-1; j>=i; j--) {
...
types::t_int find_point_op_type(rVector3d *pspecial_dir, const rMatrix3d &point_op) {
  rVector3d &special_dir=*pspecial_dir;
  types::t_real rdet=det(point_op);
  types::t_int det=iround(rdet);
  types::t_real rtrace=trace(point_op);
  types::t_int trace=iround(rtrace);
  if (fabs(rdet-(types::t_real)det)>zero_tolerance || fabs(rtrace-(types::t_real)trace)>zero_tolerance ||
      abs(trace)>3 || abs(det)>1) {
    ERRORQUIT("get_point_op_type: Not a crystallographic symmetry");
  }
  types::t_int sym_type;
  switch (abs(trace)) {
  case 3:
  ...
  if (sym_type==-2) {
    for (types::t_int axis=0; axis<3; axis++) {
      rVector3d v(0.,0.,0.);
      v(axis)=1.;
    ...
    for (types::t_int axis=0; axis<3; axis++) {
      ...
      rVector3d v(0.,0.,0.);
      v(axis)=1.;
      for (types::t_int i=0; i<period; i++) {
...
types::t_int number_of_point_op(types::t_int point_op_type, const Array<rMatrix3d> &point_op) {
  types::t_int nb=0;
  rVector3d dir;
  for (types::t_int op=0; op<point_op.get_size(); op++) {
...
void find_atom_point_group(Array<rMatrix3d> *patom_point_group,
  ...
  rMatrix3d inv_cell=!cell;
  types::t_int nb_op=0;
  Array<types::t_int> op_index(point_op.get_size());
  for (types::t_int op=0; op<point_op.get_size(); op++) {
  ...
  for (types::t_int op=0; op<nb_op; op++) {
    atom_point_group(op)=point_op(op_index(op));
  }
}

types::t_int contains_pure_translations(const Array<rMatrix3d> &point_op, const Array<rVector3d> &trans) {
  rMatrix3d id;
  id.identity();
  types::t_int nb_id=0;
  for (types::t_int op=0; op<point_op.get_size(); op++) {
...
types::t_int contains_pure_translations(const Structure &str) {
  Array<rMatrix3d> point_ops(1);
  Array<rVector3d> trans(1);
  ...
  for (types::t_int p=1; p<str.atom_pos.get_size() ; p++) {
