...
void calc_formation(Array<Real> *pfe, Array<Real> *ppure, const Array<Array<Real> > &x, const Array<Real> &e) {
  if (ppure->get_size()==0) {
    ...
    calc_convex_hull(&hull, x, Array<Real>() );
    LinkedListIterator<PolytopeFace> i(hull);
	...
	Real l2=inner_product(x(i->pts(k)),x(i->pts(k)));
	while (insertp) {
	  Real lcurp=inner_product(*insertp,*insertp);
	  if (l2>lcurp) break;
	  if (fabs(l2-lcurp)<zero_tolerance && e(i->pts(k))<(*inserte)) break;
	  insertp++;
	  inserte++;
	}
	pts.add(new Array<Real>((const Array<Real> &)(x(i->pts(k)))),insertp);
	es.add(new Real(e(i->pts(k))),inserte);
      }
    }
    ...
    //    cerr << "np=" << pts.get_size() << endl;

    Array2d<Real> lhs(ip->get_size(), ip->get_size()); 
    Array<Real> rhs(ip->get_size());
    int j;
    for (j=0; ip; ip++, ie++, j++) {
      for (int k=0; k<ip->get_size(); k++) {
	lhs(j,k)=(*ip)(k);
      }
      rhs(j)=*ie;
    }
    for (int k=0; k<lhs.get_size()(1); k++) {
      int l;
      for (l=0; l<j; l++) {
	if (fabs(lhs(l,k)-lhs(0,k))>zero_tolerance) break;
      }
      if (l==j) {
	for (int i=0; i<lhs.get_size()(1); i++) {
	  lhs(j,i)=(i==k ? 0. : lhs(0,i));
	}
	rhs(j)=rhs(0);
	j++;
      }
