...
namespace mpi
{
  ...
  {
    types::t_int n = _l.points.size();
    if( not serialize( n ) ) return false;
    if ( stage == COPYING_FROM_HERE )
      _l.points.resize(n);
    std::vector<Ising_CE::Linear_Interpolator::Point> :: iterator i_point = _l.points.begin();
    std::vector<Ising_CE::Linear_Interpolator::Point> :: iterator i_point_end = _l.points.end();
    for(; i_point != i_point_end; ++i_point )
      if (    ( not serialize( i_point->x ) )
           or ( not serialize( i_point->y ) ) ) return false;
  ...
  {
    // first serializes rank and attenuation
    if ( not serialize( _h.rank ) ) return false;
    if( not serialize( _h.attenuation ) ) return false;
    // then serializes interpolator
    if ( not serialize( _h.interpolation ) ) return false;
