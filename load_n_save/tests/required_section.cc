      ...
      template<class T_ARCHIVE>
        bool lns_access(T_ARCHIVE &_ar, load_n_save::version_type const _version)
        {
          return _ar &
            (
              lns::section("A") 
                  << ( lns::section("i",lns::tag=lns::optional) 
                           << lns::option("i", lns::action=i) )
                  << ( lns::section("hello")
...
{
  std::string str1= "<A> <i i=5/> <hello hello=\"goodbye\"/> </A>";
  std::string str2= "<A> <hello hello=\"goodbye\"/> </A>";
  std::string str3= "<A> <i i=5/> </A>";
  here::A a, b, c;
