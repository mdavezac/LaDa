...
namespace LaDa
{
  namespace CE
  {
    namespace ConstituentStrain
    {
      ...
      types::t_real Functional<T_HARMONIC> :: evaluate()
      {
        ...
        std::vector<Eigen::Vector3d> :: const_iterator i_k_vec = k_vecs.begin();
        std::vector<Eigen::Vector3d> :: const_iterator i_k_vec_end __SERIALCODE( = k_vecs.end() );
        std::vector<Eigen::Vector3d> :: const_iterator i_r_vec;
        std::vector<Eigen::Vector3d> :: const_iterator i_r_vec_begin = r_vecs.begin();
        std::vector<Eigen::Vector3d> :: const_iterator i_r_vec_end = r_vecs.end();
  ...
  #undef __ADDHERE__
      ...
      void Functional<T_HARMONIC> :: print_xml( TiXmlElement &_node ) const
      {
        ...
        std::vector<Eigen::Vector3d> :: const_iterator i_vec = k_vecs.begin();
        std::vector<Eigen::Vector3d> :: const_iterator i_end = k_vecs.end();
      ...
      bool Functional<T_HARMONIC> :: Load( const TiXmlElement &_element )
      {
        ...
        t_Harmonic harmonic;
        Eigen::Vector3d vec; types::t_real d;
