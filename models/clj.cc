    ...
    Clj :: t_Return Clj :: all_( const t_Arg& _in, t_Arg &_out )
    {
      ...
      for( t_cit i_atom1( i_atom_begin ); i_atom1 != i_atom_end; ++i_atom1, ++i_force1 )
      {
        ...
        for( t_cit i_atom2( i_atom1 + 1 ); i_atom2 != i_atom_end; ++i_atom2, ++i_force2 )
        {
          ...
          const Specie :: t_Radius rcut_squared( lj_.rcut * lj_.rcut * sigma_squared );
          ...
          for( size_t i(-lj_.mesh[0]); i < lj_.mesh[0]; ++i )
            for( size_t j(-lj_.mesh[1]); j < lj_.mesh[1]; ++j )
              for( size_t k(-lj_.mesh[2]); k < lj_.mesh[2]; ++k )
              {
                // computes distance.
                ...
                // energy -= 4.0 * lj_.scale * lj_pot( sigma_squared / rcut_squared )
                { // compute correction energy
                  ...
                  energy -= 4.e0 * lj_.scale * ( twelveth - sixth );
                }

                // energy += 4.0 * lj_.scale * lj_pot( sigma_squared / rcut_squared )
                // force += 24.0 * lj_.scale * ( 2*u^14 - u^8 ) * d
                ...
                { // compute correction energy
                  ...
                  const types::t_real ffactor( 24.e0 * lj_.scale * ( 2.e0 * twelveth - sixth ) * inv_normd );
