...
int main()
{
  ...
  LADA_DOASSERT(eq(smith_index(transform, rVector3d(2, 0.5, 0.5)), iVector3d(0, 1, 1)),
               "Incorrect smith index.\n" )
  LADA_DOASSERT(periodic_images(transform, rVector3d(1, -0.5, 0.5), rVector3d(2, 0.5, 0.5)),
              "Are not perioric images.\n")
  LADA_DOASSERT(periodic_images(transform, rVector3d(1, -0.425, 0.578), rVector3d(2, 0.575, 0.578)),
              "Are not perioric images.\n")
...
# endif
  ...
  catch( error::root &_e ) { LADA_DOASSERT(true, "Should not be here.\n"); }
  LADA_DOASSERT(result, "Did not catch exception.");
  result = false;
  try { smith_transform(unitcell, supercell); }
