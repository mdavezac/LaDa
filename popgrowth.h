  ...
  class PopGrowth : public eoUF<t_Islands&, void >
  {
    ...
    public:
      PopGrowth   ( eoEvalFunc<t_Object> &_eval, Breeder<t_Object> &_breed,
                   t_unsigned _every, t_real _gr, t_unsigned _max_pop ) 
      ...
      PopGrowth   ( const Colonize<t_Object, t_Islands> &_copy )
               : every(_copy.every), growth_rate(_copy.growth_rate), max_pop(_copy.max_pop),
      ...
      void operator()( t_Islands &_islands )
      {
        ...
        *howmany_address = &howmany;
        ...
        // ramps up every
        nb_count -= every;
        every += (t_unsigned)( (t_real) (static_cast<t_real>(every) * growth_rate) );
