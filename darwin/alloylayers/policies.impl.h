  ...
  namespace AlloyLayers
  {
    template< class T_OBJECT >
                                             ...
                                             t_Object& _object ) const
      {
        ...
        {
          const types::t_real new_norm = i_atom->pos * direction;
          if( Fuzzy::eq( last_norm, new_norm ) )
          {
            // Still in same layer
    ...
    template< class T_OBJECT >
      void Translate<T_OBJECT> :: translate( const t_Object& _object
                                             Crystal::Structure& _structure ) const
      {
        ...
        for( ; i_atom != i_atom_end __DODEBUGCODE( and i_var != i_var_end ); ++i_atom )
        {
          ...
          std::vector<bool> arrangment(layer_size, *i_var > 0 ? false: true );
          std::fill
          ( 
            arrangment.begin(),
            arrangment.begin() + size_t( ls * ( *i_var )  )
            *i_var > 0e0 ? true: false
          );
          std::shuffle( arrangment.begin(), arrangment.end(), ptr_to_rng );
