    ...
    namespace Graph
    {
        ...
        template< class T_GA_TRAITS >
          typename BullObjective<T_GA_TRAITS>::t_ScalarQuantity
            BullObjective<T_GA_TRAITS> :: evaluate_with_gradient( const t_Quantity &_q,
                                                                  t_QuantityGradients &_grad,
                                                                  t_VA_Type *_i_grad)
            {
              t_CommBase::send_quantities( _q );
              t_CommBase::send_gradients( _grad );
              t_CommBase::receive_gradients( _i_grad, gradients.size() );
              t_CommBase::receive_object( result );
              return results;
            };

        template< class T_GA_TRAITS >
          BullObjective<T_GA_TRAITS> :: evaluate_gradient( const t_Quantity &_q,
                                                           t_QuantityGradients &_grad,
                                                           t_VA_Type *_i_grad)
          {
            t_CommBase::send_quantities( _q );
        ...
        template< class T_GA_TRAITS >
          typename BullObjective<T_GA_TRAITS>::t_ScalarQuantity
            BullObjective<T_GA_TRAITS> :: evaluate_one_gradient( const t_Quantity &_q,
                                                                 t_QuantityGradients &_grad,
                                                                 types::t_unsigned _n)
            {
              t_CommBase::send_quantities( _q );
